...
class ArrayStoreTest extends CacheUnitTestCase
{
    private ArrayStore $store;

    protected function setUpCache(): void
    {
        $this->store = $this->createArrayStore('test_');
    }
    ...
    public function test_remember_caches_null_value(): void
    {
        ...
        $value1 = $this->store->remember('null_key', null, function () use (&$callCount) {
            $callCount++;
            return null;
        });

        $value2 = $this->store->remember('null_key', null, function () use (&$callCount) {
            $callCount++;
    ...
    {
        $store1 = new ArrayStore('prefix1_');
        $store2 = new ArrayStore('prefix2_');
